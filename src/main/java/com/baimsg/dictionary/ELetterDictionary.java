 ...
 **/
public class ELetterDictionary implements DictionarySuper {
    ...
    static {
        ...
        headers.put("Host", "101.37.118.121");
    }

    public ELetterDictionary(User user) {
        this.user = user;
    }
    ...
    public User login() {
        try {
            ...
            JSONObject res = new JSONObject(HttpUtils.build().
                    exePost("http://101.37.118.121/auth/login/userName",
                            JSON_RES.toString(),
                            headers
                    ));
